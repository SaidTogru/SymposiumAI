...
class ConfusionDetector:
    def __init__(self, tmp_folder_path='tmp/videoframes', confusion_threshold_seconds=5, confusion_percentage_threshold=0.7):
        # file folder 
    ...
    def is_confused(self, emotions):
        if emotions['surprise'] > self.surprise_threshold and (emotions['fear'] > self.fear_threshold or emotions['sad'] > self.sad_threshold):
            return True
        return False


    def call_llm(self, user):
        rag = RAGPipeline(
            documents_dir="src/advanced-rag/db/raw_documents",
            self_correction=False,
            text_splitter="recursive"
        )
        result = rag(f"{user} looks like confused in the last 5 seconds", addressee=user, chat_history=[], previously_retrieved_docs=[])
        print(result)
    ...
    def process_frames(self):
        ...
        for (user, user_path) in user_paths:
            current_folder = set([os.path.join(user_path, f) for f in os.listdir(user_path)])
            ...
            for frame in frames:
                try:
                    ...
                    confusion_detections = [
                        (timestamp, is_confused) for (timestamp, is_confused) in confusion_detections
                    ...
                    if confusion_detections:
                        confusion_count = sum(1 for (_, is_confused) in confusion_detections if is_confused)
                    ...
                    if len(confusion_detections) >= 10 and confusion_percentage >= self.confusion_percentage_threshold and (self.last_call is None or time.time() - self.last_call >= 3 * 60):
                        print(f"{user} detected! with {confusion_percentage}")
                        self.last_call = time.time()
                        # call the llm and retrive the result
                        # self.call_llm()
