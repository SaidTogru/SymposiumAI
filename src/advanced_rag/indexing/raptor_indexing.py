...
class RaptorIndexBuilder(IndexInterface):
    def __init__(
        self,
        ...
        self.documents_dir = os.path.join(os.getcwd(), "src", "advanced-rag", "db", "raw_documents")

        persist_base_directory = os.path.join(os.getcwd(), "src", "advanced-rag", "db", "vectorstores")
        self.persist_directory = os.path.join(
            persist_base_directory, "raptor"
        )
    ...
    def build_index(self):
        ...
        if not self.index_exists():
            ...
            loader = UnstructuredPDFLoader(
                file_path=self.documents_dir,
            )


            data = loader.load()
            leaf_texts = [doc.page_content for doc in data]

            # Build tree
            results = self._recursive_embed_cluster_summarize(leaf_texts, level=1, n_levels=3)
            ...
            self.vectorstore = Chroma.from_texts(
                texts=all_texts,
                collection_name="raptor-chroma-index",
                embedding=self.embd,
    ...
    def _perform_clustering(
        ...
        for i in range(n_global_clusters):
            ...
            _global_cluster_embeddings_ = embeddings[
                np.array([i in gc for gc in global_clusters])
            ]

            if len(_global_cluster_embeddings_) == 0:
                continue
            if len(_global_cluster_embeddings_) <= dim + 1:
                # Handle small clusters with direct assignment
                local_clusters = [np.array([0]) for _ in _global_cluster_embeddings_]
                n_local_clusters = 1
            else:
                # Local dimensionality reduction and clustering
                reduced_embeddings_local = self._local_cluster_embeddings(
                    _global_cluster_embeddings_, dim
                )
            ...
            for j in range(n_local_clusters):
                _local_cluster_embeddings_ = _global_cluster_embeddings_[
                    np.array([j in lc for lc in local_clusters])
                ]
                indices = np.where(
                    (embeddings == _local_cluster_embeddings_[:, None]).all(-1)
                )[1]
                for idx in indices:
    ...
    def _embed_cluster_texts(self, texts):
        """
        Embeds a list of texts and clusters them, returning a DataFrame with texts, their embeddings, and cluster labels.

        This function combines embedding generation and clustering into a single step. It assumes the existence
        of a previously defined `_perform_clustering` function that performs clustering on the embeddings.
        ...
        """
        text_embeddings_np = self._embed(texts)  # Generate embeddings
    ...
    def _fmt_txt(self, df: pd.DataFrame) -> str:
        """
...
# llm, embd = setup_models(llm_config, embedding_config)
# docs2feed_path = "./data/10 GWE Data/02_ProcessedDocuments/docs2feed small.jsonl"
# raptor = RaptorIndexBuilder(docs2feed_path, embd, llm, "")
# raptor.build_index()
# vectorstore = raptor.get_vectorstore()
